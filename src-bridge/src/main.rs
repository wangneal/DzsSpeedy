...
use windows::Win32::Foundation::{
    CloseHandle, GetLastError, BOOL, ERROR_ALREADY_EXISTS, ERROR_PIPE_CONNECTED, HANDLE, HINSTANCE,
    HWND, INVALID_HANDLE_VALUE, LPARAM,
};
...
use windows::Win32::System::Threading::{
    CreateMutexW, CreateRemoteThread, GetCurrentProcess, IsWow64Process2, OpenProcess,
    QueryFullProcessImageNameW, WaitForSingleObject, PROCESS_CREATE_THREAD, PROCESS_NAME_WIN32,
...
use windows::Win32::UI::WindowsAndMessaging::{
    EnumWindows, GetWindowThreadProcessId, PostThreadMessageW, SetWindowsHookExW,
...
fn retained_hook_count_for_pid(pid: u32) -> usize {
    retained_hooks()
        ...
        .map(|hooks| {
            hooks
                .iter()
                .filter(|(hook_pid, _)| *hook_pid == pid)
                .count()
        })
        .unwrap_or(0)
}
...
fn query_process_is64(pid: u32) -> Result<bool, String> {
    unsafe {
        ...
        match native_machine {
            IMAGE_FILE_MACHINE_AMD64 | IMAGE_FILE_MACHINE_ARM64 => Ok(true),
...
unsafe extern "system" fn enum_windows_for_pid(hwnd: HWND, lparam: LPARAM) -> BOOL {
    ...
    if window_pid == ctx.pid && thread_id != 0 {
        if !ctx.threads.contains(&thread_id) {
...
fn target_window_threads(pid: u32) -> Vec<u32> {
    ...
    // Some targets have no top-level window, keep their windows on another
    // desktop, or create the GUI later. Fall back to every thread owned by the
    // target; SetWindowsHookEx/PostThreadMessage will reject unsuitable ones.
    let snapshot = match unsafe { CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, 0) } {
        Ok(snapshot) => snapshot,
        Err(e) => {
            dbg_log(&format!(
                "target_window_threads: thread snapshot failed pid={pid} err={e:?}"
            ));
            return Vec::new();
        }
    };
    ...
    unsafe {
        if Thread32First(snapshot, &mut entry).is_ok() {
            loop {
                if entry.th32OwnerProcessID == pid && !ctx.threads.contains(&entry.th32ThreadID) {
                    ctx.threads.push(entry.th32ThreadID);
                }
...
fn try_windows_hook_x86(pid: u32, dll_path: &str) -> Result<(), String> {
    ...
    dbg_log(&format!(
        "try_windows_hook_x86: pid={} window_threads={:?}",
        pid, threads
    ));
    if threads.is_empty() {
        return Err(format!(
            "no target GUI/thread candidate found for pid={pid}"
        ));
    }
    ...
    for thread_id in threads {
        ...
        match hook {
            Ok(h) => {
                dbg_log(&format!(
                    "try_windows_hook_x86: SetWindowsHookExW OK thread={} hook={:?}",
                    thread_id, h
                ));
                hooks.push(h);
                match unsafe { PostThreadMessageW(thread_id, WM_NULL, None, None) } {
                    Ok(()) => dbg_log(&format!(
                        "try_windows_hook_x86: PostThreadMessageW OK thread={thread_id}"
                    )),
                    Err(e) => dbg_log(&format!(
                        "try_windows_hook_x86: PostThreadMessageW FAILED thread={thread_id} err={e:?}"
                    )),
                }
            }
            Err(e) => {
                dbg_log(&format!(
                    "try_windows_hook_x86: SetWindowsHookExW FAILED thread={} err={:?}",
                    thread_id, e
                ));
            }
        }
    ...
    Err(format!(
        "SetWindowsHookExW installed but DzsSpeedy.{pid} mapping not created within 5s"
    ))
}
// ── Core operations ──────────────────────────────────────────────────────
...
fn do_inject(pid: u32) -> Result<(), String> {
    ...
    if !is64 {
        match try_windows_hook_x86(pid, &dll_str) {
            Ok(()) => {
                dbg_log(&format!(
                    "do_inject pid={}: SetWindowsHookEx + FileMapping OK",
                    pid
                ));
            ...
            Err(e) => {
                dbg_log(&format!(
                    "do_inject pid={}: SetWindowsHookEx path failed: {}",
                    pid, e
                ));
        ...
        match try_ldr_load_dll_x86(pid, &h_proc, &dll_str) {
            Ok(()) => {
                ...
                if do_status(pid).is_some() {
                    dbg_log(&format!(
                        "do_inject pid={}: LdrLoadDll + FileMapping OK",
                        pid
                    ));
                    unsafe {
            ...
            Err(e) => {
                dbg_log(&format!(
                    "do_inject pid={}: LdrLoadDll path failed: {}",
                    pid, e
                ));
    ...
    if let Err(e) = try_inject_impl(pid, &h_proc, &dll_str, "LoadLibraryW", true) {
        ...
        if do_status(pid).is_none() {
            ...
            dbg_log(&format!(
                "do_inject pid={}: LoadLibraryW + FileMapping OK",
                pid
            ));
    ...
    if let Err(e) = try_inject_impl(pid, &h_proc, &dll_str, "LoadLibraryA", false) {
        ...
        if do_status(pid).is_none() {
            ...
            dbg_log(&format!(
                "do_inject pid={}: LoadLibraryA + FileMapping OK",
                pid
            ));
...
fn remote_module_proc(
    ...
    .ok_or_else(|| format!("GetProcAddress {module_name}!{proc_name}"))?
        as usize;
...
fn try_ldr_load_dll_x86(pid: u32, h_proc: &HANDLE, dll_path: &str) -> Result<(), String> {
    let (ldr_load_dll, local_ldr_load_dll, ldr_rva) =
        remote_module_proc(pid, "ntdll.dll", "LdrLoadDll")?;
    dbg_log(&format!(
        "try_ldr_load_dll_x86: pid={} LdrLoadDll local=0x{:x} rva=0x{:x} remote=0x{:x}",
    ...
    if ldr_load_dll > u32::MAX as usize {
        return Err(format!(
            "LdrLoadDll remote address 0x{ldr_load_dll:x} is not 32-bit"
        ));
    }
    ...
    let result = (|| -> Result<(), String> {
        ...
        let h_thread = unsafe {
            CreateRemoteThread(
                *h_proc,
                None,
                0,
                Some(std::mem::transmute(code_remote)),
                Some(remote_base),
                0,
                None,
            )
        ...
        dbg_log(&format!(
            "try_ldr_load_dll_x86: exit=0x{:08x} status=0x{:08x} hmodule=0x{:08x} dll={}",
    ...
    unsafe {
        let _ = VirtualFreeEx(*h_proc, remote_base, 0, MEM_RELEASE);
    }
    result
}
...
fn try_inject_impl(
    ...
    let path_bytes: Vec<u8> = if wide {
        ...
        dll_path.as_bytes().iter().copied().chain([0u8]).collect()
    };
    ...
    dbg_log(&format!(
        "try_inject_impl: pid={} {} local=0x{:x} rva=0x{:x} remote=0x{:x}",
    ...
    if is_failure {
        ...
        dbg_log(&format!(
            "try_inject_impl: {} returned exit_code=0x{:08x} gle={} dll={}",
    ...
    dbg_log(&format!(
        "try_inject_impl: {} success (hmodule=0x{:x}) dll={}",
...
fn do_eject(pid: u32) -> Result<(), String> {
    ...
    let h_shutdown = unsafe {
        CreateRemoteThread(
            ...
            None,
            0,
            Some(std::mem::transmute(remote_shutdown)),
            None,
            0,
            None,
        )
    ...
    unsafe {
        WaitForSingleObject(h_shutdown, 5000);
    }
    ...
    let kernel32 = unsafe { GetModuleHandleW(PCWSTR::from_raw(to_wide("kernel32.dll").as_ptr())) }
    ...
    let h_thread = unsafe {
        CreateRemoteThread(
            ...
            None,
            0,
            Some(std::mem::transmute(free_lib)),
            Some(h_mod_ptr),
            0,
            None,
        )
...
fn do_status(pid: u32) -> Option<bool> {
    // The mapping is the bridge/DLL handshake and has a permissive DACL. It is
    // more reliable than module snapshots, which need extra access on some
    // processes and can fail for ordinary users even when the mapping is readable.
