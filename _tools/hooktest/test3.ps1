...
$out = Join-Path $dir 'result3.txt'
"=== start $(Get-Date -Format o) ===" | Out-File $out -Encoding utf8

Add-Type -AssemblyName System.IO.Pipes

$bridge = 'D:\Program Files\DzsSpeedy\bridge64.exe'
$existing = Get-Process -Name bridge64 -ErrorAction SilentlyContinue
if ($existing) {
...
$pipe = New-Object System.IO.Pipes.NamedPipeClientStream('.', 'DzsSpeedyBridge64', [System.IO.Pipes.PipeDirection]::InOut)
...
Get-Content "$env:TEMP\dzsspeedy-bridge.log" -Tail 60 -ErrorAction SilentlyContinue | Out-File $out -Append -Encoding utf8
